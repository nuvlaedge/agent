...
class NuvlaBoxCommonTestCase(unittest.TestCase):
    ...
    def test_init(self):
        self.assertEqual(self.obj.data_volume, "/srv/nuvlabox/shared",
                         'Default NuvlaBox data volume path was not set correctly')
        ...
        self.assertIsNotNone(self.obj.nuvlabox_engine_version,
    ...
    @mock.patch('agent.common.NuvlaBoxCommon.KubernetesClient')
    ...
    @mock.patch('os.path.exists')
    def test_set_nuvlabox_id(self, mock_exists):
    ...
    def test_get_api_keys(self):
        ...
        os.environ['NUVLABOX_API_KEY'] = 'api-key'
        os.environ['NUVLABOX_API_SECRET'] = 'api-secret'
        self.assertEqual(self.obj.get_api_keys(), ('api-key', 'api-secret'),
                         'Unable to fetch API keys from env')

        for key in ['NUVLABOX_API_KEY', 'NUVLABOX_API_SECRET']:
    ...
    @mock.patch('agent.common.NuvlaBoxCommon.NuvlaBoxCommon.api')
    ...
    @mock.patch('agent.common.NuvlaBoxCommon.Popen')
    ...
    @mock.patch('agent.common.NuvlaBoxCommon.NuvlaBoxCommon.read_json_file')
    @mock.patch('os.path.exists')
    def test_get_nuvlabox_version(self, mock_exists, mock_read_json):
        # if the version is already an attribute of the class, just give back its major version
        major = 2
        self.obj.nuvlabox_engine_version = f'{major}.1.0'
        self.assertEqual(self.obj.get_nuvlabox_version(), major,
                         'Unable to infer NBE major version')

        # otherwise, get it from the data volume
        self.obj.nuvlabox_engine_version = None
        mock_exists.return_value = True
    ...
    @mock.patch('os.path.exists')
    @mock.patch('agent.common.NuvlaBoxCommon.NuvlaBoxCommon.shell_execute')
    ...
    @mock.patch('agent.common.NuvlaBoxCommon.NuvlaBoxCommon.write_vpn_conf')
    @mock.patch('agent.common.NuvlaBoxCommon.NuvlaBoxCommon.prepare_vpn_certificates')
